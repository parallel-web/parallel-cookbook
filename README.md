...
| [**Datacenter Monitor**](typescript-recipes/parallel-datacenter-map) | Live map of ~2,700 US datacenters, each discovered, enriched across 25 fields, and AI-impact classified by the Task API, with 31 event-stream monitors plus 200 daily snapshot monitors watching the web. Per-field reasoning and citations via `output.basis`. Facility discovery uses shard-by-state plus `previous_interaction_id` pagination. | `Task` `Task Group` `Monitors` `Webhooks` `SSE` | Next.js · Vercel · Leaflet | [Live](https://datacenter-demo.app) |
...
| Recipe | Description | APIs | Stack | Demo |
| --- | --- | --- | --- | --- |
| [**Gemini + Parallel Enrichment**](python-recipes/gemini_ai_demo) | Fill missing company, people, and product details with Gemini and Parallel, using Google’s native SDK. Returns structured records with sources attached. | `Search` | Python · Google Vertex AI | – |
