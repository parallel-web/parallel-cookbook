...
3. **Don't touch the output JSON schemas or the `field-basis` beta header** in
   `parallel_client.py` without updating `types.ts` and `to_research_brief()` in lockstep —
   the field names map one-to-one to the UI.
...
The CRM is **pluggable**. The signals pipeline asks one question about each company —
*already in our CRM? active deals? who owns it?* — through the small contract in
