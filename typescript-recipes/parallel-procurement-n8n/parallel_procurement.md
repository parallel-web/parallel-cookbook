...
**Traceable down to the source URL.** Every Parallel deep research result and every V1 monitor event ships an `output.basis` array — one entry per output field with reasoning, confidence, and a list of citation URLs. The risk scorer groups those entries by dimension, picks the highest-confidence citation per triggered dimension, and lifts them into a flat `top_citations` array on the assessment. From there:

- The audit log row carries `top_citation_url`, `top_citation_title`, and `confidence` columns so the Sheets export alone tells you which article or filing tipped the scorer into HIGH or CRITICAL.
- CRITICAL/HIGH Slack alerts, the ad-hoc thread reply, and the monitor alert all append a `Sources:` block with up to 3 markdown links.

The deterministic scoring cascade — severity counts, override rules, `risk_tier_override` floor — is unchanged. Basis is purely an audit and observability layer on top, but it's the layer that turns "the AI flagged this vendor" into "the SEC filing on 2026-03-05 cited here was the trigger."
...
- **A single combined workflow** (50 nodes, zero cross-workflow wiring) ready to import into n8n
