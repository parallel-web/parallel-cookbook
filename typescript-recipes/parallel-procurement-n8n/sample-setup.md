...
Every Parallel call in the system — Task Group submission, monitor creation, monitor event resolution, ad-hoc deep research, monitor cancellation — runs through the official `parallel-web` SDK on the V1 Monitor + Task APIs. Every assessment ships with `top_citations` lifted from `output.basis`, so the Slack alert and the audit-log row both link directly to the source URLs the scoring engine used.
