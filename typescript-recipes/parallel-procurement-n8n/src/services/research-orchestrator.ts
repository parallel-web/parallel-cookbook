...
export class ResearchOrchestrator {
  ...
  async runScheduledResearch(
    ...
    const allBasis = new Map<string, BasisEntry[]>();
    const allRunIds = new Map<string, string>();
    ...
    for (const batch of batches) {
      ...
      for (const [domain, basis] of batchResult.basis) {
        allBasis.set(domain, basis);
      }
      for (const [domain, runId] of batchResult.run_ids) {
        allRunIds.set(domain, runId);
      }
  ...
  async executeBatch(batch: VendorBatch): Promise<BatchResult> {
    ...
    // Map results back to vendor domains. We split content + basis so the
    // scorer can consume both, and we preserve run_ids for the audit row.
    const results = new Map<string, DeepResearchOutput>();
    const basis = new Map<string, BasisEntry[]>();
    const runIdsByDomain = new Map<string, string>();
    ...
    for (const run of groupResults) {
      ...
      if (run.status === "completed" && run.output) {
        ...
        basis.set(
          vendorDomain,
          (run.output.basis ?? []) as BasisEntry[],
        );
        runIdsByDomain.set(vendorDomain, run.run_id);
      } else {
        failures.push({
  ...
  async processResults(
    ...
    vendors: Vendor[],
    basisByDomain?: Map<string, BasisEntry[]>,
    runIdsByDomain?: Map<string, string>,
    ...
    for (const [domain, output] of results) {
      ...
      try {
        const basis = basisByDomain?.get(domain);
        const assessment = this.riskScorer.scoreDeepResearch(
          output,
          { risk_tier_override: vendor.risk_tier_override },
          basis,
        );

        // Route to Slack (uses assessment.top_citations for the Sources block)
        await this.routeToSlack(assessment, vendor, output);

        // Audit log — surface the top citation alongside the existing
        // fields so the row alone explains "why was this flagged".
        ...
        await this.auditLogger.logAssessment({
          ...
          run_id: runIdsByDomain?.get(domain) ?? "",
