...
export class SlackCommandHandler {
  ...
  async handleWebhookCallback(payload: TaskWebhookPayload): Promise<void> {
    ...
    const basis = (result.output.basis ?? []) as BasisEntry[];

    // Honor the vendor's risk_tier_override floor + plumb basis so the
    // thread reply renders source URLs in its Sources block.
    const assessment = this.riskScorer.scoreDeepResearch(
      researchOutput,
      { risk_tier_override: pending.vendor.risk_tier_override },
      basis,
    );
