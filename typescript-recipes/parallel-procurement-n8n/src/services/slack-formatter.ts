...
export class SlackFormatter {
  ...
  formatCriticalAlert(
    ...
    const sourcesBlock = formatSourcesBlock(assessment);
  ...
  formatMonitorAlert(
    ...
    blocks.push(contextBlock(`Detected: ${new Date().toISOString()}`));
...
// Render `assessment.top_citations` as a bulleted Slack section so the
// alert links straight to the source URLs the scoring engine relied on.
// Returns null when the assessment carries no citations (LOW risk
// assessments, monitor events from output-less monitors, etc.) — we want
// silent rendering, not an empty block.
function formatSourcesBlock(assessment: RiskAssessment): SlackBlock | null {
  const citations = assessment.top_citations;
  if (!citations || citations.length === 0) return null;

  const lines = citations.map((c) => {
    const labelText = c.title?.trim() ? c.title.trim() : c.url;
    const label = sanitizeSlackLinkText(labelText);
    const confidence = c.confidence ? ` _(confidence: ${c.confidence})_` : "";
    return `\u2022 <${c.url}|${label}> — _${c.dimension}_${confidence}`;
  });

  return sectionBlock(`*Sources:*\n${lines.join("\n")}`);
}

// Slack link labels can't contain `<`, `>`, or `|` without breaking the
// mrkdwn parser; sanitize defensively.
