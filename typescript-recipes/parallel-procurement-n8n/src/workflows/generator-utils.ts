...
// ── Vendor name escaping (finding 15) ──────────────────────────────────────
//
// The five monitor query templates wrap the vendor name in literal double
// quotes (`"<vendor_name>" lawsuit OR ...`). A name with an embedded quote
// (e.g. `Acme "AI" Inc`) produces unterminated quote pairs and breaks
// Parallel's search syntax. We collapse any embedded `"` to `'` and trim
// double whitespace before interpolation.
//
// The TS export is used by the dashboard's generateMonitorQueries; the
// JS-source helper string is interpolated into the n8n Code nodes so they
// run the same logic at workflow execution time.
...
/**
 * JS source for the same function as a one-liner string. Interpolated at
 * the top of every Code node body that builds monitor query templates so
 * src and runtime stay in lock-step.
 */
export const ESCAPE_VENDOR_NAME_JS = `function escapeVendorName(name) {
...
/**
 * `parallelSdkCodeNode` is the canonical wrapper for any Parallel API
 * call from inside an n8n Code node. We prepend the same boilerplate
 * (`require('parallel-web')` + client construction with the workspace
 * `PARALLEL_API_KEY` env var) to every snippet so individual generators
 * don't have to repeat it. Self-hosted n8n needs
 * `NODE_FUNCTION_ALLOW_EXTERNAL=parallel-web` for `require()` to succeed;
 * see SETUP.md.
 */
export function parallelSdkCodeNode(
  name: string,
  sdkBody: string,
  position: [number, number],
): N8nNode {
  const wrapped = `${SDK_PREAMBLE}\n${sdkBody}`;
  return createNode(name, "code", position, { mode: "runOnceForAllItems", jsCode: wrapped }, 2);
}

const SDK_PREAMBLE = `// Parallel TypeScript SDK — call client.monitor.* / client.taskRun.* /
