...
let mockCancelMonitor: ReturnType<typeof vi.fn>;
...
beforeEach(() => {
  ...
  mockCancelMonitor = vi.fn().mockResolvedValue({
    monitor_id: "mon_x",
    type: "event_stream",
    frequency: "1d",
    processor: "lite",
  ...
  mockClient = {
    ...
    cancelMonitor: mockCancelMonitor,
...
describe("deployMonitors", () => {
  ...
  it("passes V1 settings.output_schema to createMonitor", async () => {
    const manager = createManager();
    ...
    expect(mockCreateMonitor).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        settings: expect.objectContaining({
          ...
          advanced_settings: expect.objectContaining({ location: expect.any(String) }),
        }),
      }),
    );
  });
...
describe("removeMonitors", () => {
  it("calls cancelMonitor (V1) for each ID", async () => {
    const manager = createManager();

    await manager.removeMonitors(["mon_1", "mon_2", "mon_3"]);

    expect(mockCancelMonitor).toHaveBeenCalledTimes(3);
    expect(mockCancelMonitor).toHaveBeenCalledWith("mon_1");
    expect(mockCancelMonitor).toHaveBeenCalledWith("mon_2");
    expect(mockCancelMonitor).toHaveBeenCalledWith("mon_3");
  });
  ...
  it("handles empty array", async () => {
    ...
    expect(mockCancelMonitor).not.toHaveBeenCalled();
  });
});
...
describe("applyReconciliation", () => {
  it("creates and deletes monitors as specified", async () => {
    ...
    // 2 monitors cancelled
    expect(mockCancelMonitor).toHaveBeenCalledTimes(2);
