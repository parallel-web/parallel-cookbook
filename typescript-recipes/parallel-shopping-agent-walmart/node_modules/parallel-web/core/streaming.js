...
class Stream {
    ...
    static fromSSEResponse(response, controller, client) {
        ...
        async function* iterator() {
            if (consumed) {
                throw new error_1.ParallelError('Cannot iterate over a consumed stream, use `.tee()` to split the stream.');
            }
            consumed = true;
            let done = false;
            try {
...
async function* _iterSSEMessages(response, controller) {
    if (!response.body) {
        ...
        if (typeof globalThis.navigator !== 'undefined' &&
            ...
            throw new error_1.ParallelError(`The default react-native fetch implementation does not support streaming. Please use expo/fetch: https://docs.expo.dev/versions/latest/sdk/expo/#expofetch-api`);
        }
        throw new error_1.ParallelError(`Attempted to iterate over a response with no body`);
    }
