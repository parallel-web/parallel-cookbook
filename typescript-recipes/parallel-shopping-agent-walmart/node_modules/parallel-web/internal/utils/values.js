...
const ensurePresent = (value) => {
    if (value == null) {
        throw new error_1.ParallelError(`Expected a value to be given but received ${value} instead.`);
    }
    return value;
};
...
const validatePositiveInteger = (name, n) => {
    if (typeof n !== 'number' || !Number.isInteger(n)) {
        throw new error_1.ParallelError(`${name} must be an integer`);
    }
    if (n < 0) {
        throw new error_1.ParallelError(`${name} must be a positive integer`);
    }
    return n;
};
...
const coerceInteger = (value) => {
    ...
    throw new error_1.ParallelError(`Could not coerce ${value} (type: ${typeof value}) into a number`);
};
