...
This document is the technical source of truth for the Copytree article. Copy concepts and short excerpts from the linked, type-checked source; do not maintain a second implementation in the article.
...
5. Run a focused investigation when a changed risk field is at or above policy before or after the change, or when the vendor's configured risk floor meets that threshold.
...
Use the current official TypeScript SDK, v1 Task API, and Monitor API. Remove placeholder alpha/beta endpoint calls and any snippet that depends on helpers absent from the published source.
...
- Baseline lifecycle: [`VendorIntelligence.bootstrap`](../src/vendor-intelligence.ts). Snapshot Monitor parameters live in [`buildSnapshotMonitorParams`](../src/schema.ts), and `ensureMonitor` passes them to `client.monitor.create` after the baseline completes.
- Snapshot reconstruction and evidence replacement: [`reconstructSnapshotEvent`](../src/snapshot-events.ts).
...
- SDK boundary: [`createParallelPort`](../src/parallel-sdk-adapter.ts).
...
`npm run test:live` is opt-in and consumes credits. It exercises the production bootstrap, update-check, and cleanup lifecycle, then independently confirms Monitor cancellation. A real material change is nondeterministic, so the article should identify fixture tests—not the live test—as proof of the change-to-follow-up branch.

Bootstrap is additive: omitting a saved vendor does not cancel its Monitor. The article should show scoped cleanup with `npm run cleanup -- --vendor example.com`, and describe `--retry-failed` as an explicit opt-in for terminal Task failures. Normal timeouts and transport failures resume the saved Task automatically.
